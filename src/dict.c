...
int dictExpand(dict *d, unsigned long size)
{
    ...
    n.size = realsize; // 16 10000
    n.sizemask = realsize - 1; // 15 1111

    // T = O(N)
    n.table = zcalloc(realsize * sizeof(dictEntry *)); // 16个数组
    n.used = 0;
...
{
    unsigned int iKey = *(unsigned int *)key; // 取key的地址
    return dictIntHashFunction(iKey); // 计算出一个hash值
}
...
int main(int argc, char **argv)
{
    ...
    if ((type = zmalloc(sizeof(*type))) == NULL) return -1;
    type->hashFunction = hashFunction;
    type->keyCompare = keyCompare;
    type->keyDup = keyDup;
    type->valDup = valDup;
    type->keyDestructor = keyDestructor;
    type->valDestructor = valDestructor;
